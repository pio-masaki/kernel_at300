				...
				struct cpufreq_policy *policy)
{
	...
	unsigned int qmin = min((unsigned int)pm_qos_request(PM_QOS_CPU_FREQ_MIN), data->max);
	unsigned int qmax = max((unsigned int)pm_qos_request(PM_QOS_CPU_FREQ_MAX), data->min);
