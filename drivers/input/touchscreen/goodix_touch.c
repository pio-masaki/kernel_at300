...
#define CMD_READ_CHIP_TYPE         6   //获取芯片型号,added by Scott,2011-09-15
#define CMD_NORMAL_MODE            7   //切换到正常的坐标读取模式，added by Scott,2011-09-27
...
char chip_type = 0;
static struct regulator *ts_en = NULL;
...
static int goodix_init_panel(struct goodix_ts_data *ts)
{
	...
	0x65,0x00,0x10,0x00,0x10,0x00,0x0A,0x61,0x01,0x00,0x0F,0x28,0x02,
	0x48,0x90,0x00,0x00,0x15,0x00,0x00,0x08,0x10,0x10,0x53,0x37,0x00,
	0x00,0x0B,0x0A,0x09,0x08,0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00,
	0xFF,0xFF,0xFF,0xFF,0x0A,0x09,0x08,0x07,0x06,0x05,0x04,0x03,0x02,
	0x01,0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x32,0x3C,0x00,0x00,
...
COORDINATE_POLL:
	if((ts->int_trigger_type> 1)&& (gpio_get_value(irq_to_gpio(ts->client->irq)) != (ts->int_trigger_type&0x01)))
	{
		goto NO_ACTION;
	}		
...
#ifdef CONFIG_ADB_UPDATE
static DEVICE_ATTR(update, S_IWUSR | S_IRUGO , goodix_debug_update_show, goodix_debug_update_store);
...
static int goodix_ts_regulator(int ast_touch_enable)
{
	...
	if (ast_touch_enable)
	{	
		ts_en = regulator_get(NULL, "vdd_3v3_ts");
		if (ts_en == NULL){
			if (WARN_ON(IS_ERR(ts_en))) {
				pr_err("%s: couldn't get regulator vdd_3v3_ts: %ld\n",
...
works_in_polling_mode:
...
#endif
#ifdef CONFIG_TOUCHSCREEN_GOODIX_IAP
	goodix_proc_entry = create_proc_entry("goodix-update", 0600, NULL);
    ...
    init_wr_node(client);
#endif
	dev_info(&client->dev,"Start %s in %s mode. Driver Modify Date:2011-08-11\n", 
	...
	while(length > 0)
	{
		...
		if(ret > 0)
		{
			for(count = 0; count < ret;  count++)
			{
				...
				if (buffer[count] == 0x47 && buffer[count+1] == 0x54 &&
				    buffer[count+2] == 0x38 && buffer[count+3] == 0x31 &&
				    buffer[count+4] == 0x58 && buffer[count+5] == 0x4E &&
				    buffer[count+6] == 0x49 && buffer[count+7] == 0x5F)
					check_version = 1;
			}
		}
		else 
...
static int gt81x_read_regs(struct i2c_client *client, u8 reg, u8 buf[], unsigned len)
{
	struct i2c_msg msgs[2];
	int ret=-1;
	int retries = 0;
...
static int gt81x_set_regs(struct i2c_client *client, u8 reg, u8 const buf[], unsigned short len)
{
	struct i2c_msg msg;
	int ret=-1;
	int retries = 0;
...
int gt81x_update_fw(void* arg)
{
	...
	ret = gt81x_update_write_fw(client, gt81x_fw, /*sizeof(gt81x_fw)*/fw_len);
	if(ret < 0)
		return ret;
	dev_info(&client->dev, "done!\n");
