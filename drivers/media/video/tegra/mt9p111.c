...
#define SEQ_COMMAND_ADDR                0x8404
#define SEQ_REFRESH_MODE_VAL            0x06
#define SEQ_DONE_VAL                    0x00
			    ...
			    struct mt9p111_mode *mode)
{
	...
	pr_info("%s: xres %d yres %d mode = %d\n", __func__, mode->xres,
		mode->yres, mode->mode);
	if (mode->mode == MT9P111_MODE_CAPTURE) {
		mode_table_status = MT9P111_MODE_CAPTURE;
	} else if (mode->xres == 1280 && mode->yres == 960 &&
		mode->mode == MT9P111_MODE_PREVIEW) {
		mode_table_status = MT9P111_MODE_PREVIEW;
	} else {
	...
	if (mode_table_status_last == MT9P111_MODE_UNINITED) {
...
#ifdef CONFIG_MACH_AVALON
		if (system_rev < 3)
			mt9p111_write_table(info->i2c_client,
					    orientation_normal);
#endif
		...
		if (mode_table_status == MT9P111_MODE_CAPTURE) {
			  ...
			  unsigned long arg)
{
	...
	case MT9P111_IOCTL_SET_AF_TRIGGER:
		if (mode_table_status == MT9P111_MODE_UNINITED || !arg ||
			mt9p111_read_reg8(info->i2c_client, AF_PROGRESS_ADDR))
			break;

		if (touch_active) {
			mt9p111_write_reg8(info->i2c_client, SEQ_COMMAND_ADDR,
					   SEQ_REFRESH_MODE_VAL);
			mt9p111_poll_reg8(info->i2c_client, SEQ_COMMAND_ADDR,
					  SEQ_DONE_VAL, 0xFF);
			touch_active = 0;
		}
	...
	case MT9P111_IOCTL_GET_FLASH_STATUS:
		ret = mt9p111_read_reg8(info->i2c_client,
					FLASH_STATUS_ADDR);
		break;
	...
	case MT9P111_IOCTL_SET_EXPOSURE_RECT:
		...
		if (rect.width == 0 && rect.height == 0) {
			if (exposoure_rect[1].val == exposoure_rect_deafult[1].val &&
				exposoure_rect[2].val == exposoure_rect_deafult[2].val &&
				exposoure_rect[3].val == exposoure_rect_deafult[3].val &&
				exposoure_rect[4].val == exposoure_rect_deafult[4].val)
				break;
	...
	case MT9P111_IOCTL_SET_AF_RECT:
		...
		if (rect.width == 0 && rect.height == 0) {
			if (focus_rect[1].val == focus_rect_deafult[1].val &&
				focus_rect[2].val == focus_rect_deafult[2].val &&
				focus_rect[3].val == focus_rect_deafult[3].val &&
				focus_rect[4].val == focus_rect_deafult[4].val)
				break;

			/* set back to deafult */
			focus_rect[1].val = focus_rect_deafult[1].val;
			focus_rect[2].val = focus_rect_deafult[2].val;
			focus_rect[3].val = focus_rect_deafult[3].val;
			focus_rect[4].val = focus_rect_deafult[4].val;
		} else {
