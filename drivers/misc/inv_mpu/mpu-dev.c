...
struct mpu_private_data {
	...
	struct regulator *sensor_1v8_reg;
	struct regulator *sensor_3v3_reg;
};
...
int mpu_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	/* Turning on regulator power */
   	mpu->sensor_3v3_reg = regulator_get(NULL, "vdd_3v3_sensor");
    	if (IS_ERR_OR_NULL(mpu->sensor_3v3_reg)) {
     		dev_err(&client->adapter->dev, "couldn't get regulator vdd_3v3_sensor\n");
      		res = -ENODEV;
      		goto out_get_regulator_3v3;
    	}
    	regulator_enable(mpu->sensor_3v3_reg);
    
    	mpu->sensor_1v8_reg = regulator_get(NULL, "vdd_1v8_sensor");
    	if (IS_ERR_OR_NULL(mpu->sensor_1v8_reg)) {
      		dev_err(&client->adapter->dev, "couldn't get regulator vdd_1v8_sensor\n");
      		res = -ENODEV;
      		goto out_get_regulator_1v8;
    	}
    	regulator_enable(mpu->sensor_1v8_reg);
...
out_get_regulator_1v8:
	regulator_put(mpu->sensor_1v8_reg);
out_get_regulator_3v3:
    	regulator_put(mpu->sensor_3v3_reg);
...
static int mpu_remove(struct i2c_client *client)
{
	...
	/* Turning off regulator power */
    	if (mpu->sensor_1v8_reg)
      		regulator_put(mpu->sensor_1v8_reg);
    	if (mpu->sensor_3v3_reg)
      		regulator_put(mpu->sensor_3v3_reg);
