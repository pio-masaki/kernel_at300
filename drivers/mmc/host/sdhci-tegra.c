...
#define SDHOST_HIGH_VOLT_MIN	2850000
...
static void sdhci_status_notify_cb(int card_present, void *dev_id)
{
	...
	printk(KERN_INFO "%s: %s: card_present %d\n", mmc_hostname(sdhci->mmc),
...
static irqreturn_t carddetect_irq(int irq, void *data)
{
	...
	printk(KERN_INFO "%s: present=%d\n", __func__, tegra_host->card_present);
	...
	*/
	...
	if (!plat->mmc_data.built_in) {
		...
		if (IS_ERR_OR_NULL(tegra_host->vdd_slot_reg)) {
			...
			if (tegra_host->vdd_slot_reg) {
#ifdef CONFIG_MACH_SPHINX
				rc = regulator_set_voltage(tegra_host->vdd_slot_reg, 2700000, 3600000);
				if (rc) {
					dev_err(mmc_dev(host->mmc), "%s regulator_set_voltage failed: %d",
					"vddio_sd_slot", rc);
				}
	...
	/* enable emmc and wifi dpd mode for sphinx and avalon,
	     but only enable emmc dpd mode for titan,
	*/
#if defined(CONFIG_MACH_AVALON) || defined(CONFIG_MACH_SPHINX)
	if (plat->mmc_data.built_in)
		tegra_host->dpd = tegra_io_dpd_get(mmc_dev(host->mmc));
#else
	if (plat->is_8bit && plat->mmc_data.built_in)
		tegra_host->dpd = tegra_io_dpd_get(mmc_dev(host->mmc));
#endif
