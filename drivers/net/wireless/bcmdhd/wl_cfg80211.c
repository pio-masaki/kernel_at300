	...
	struct cfg80211_connect_params *sme)
{
	...
	*/
	...
	if (security_wifi_associate(dev->name, ext_join_params->assoc.bssid.octet, ext_join_params->ssid.SSID, ext_join_params->ssid.SSID_len)) {
		err = -EINVAL;
		wl_clr_drv_status(wl, CONNECTING, dev);
		goto exit;
	}
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	//==================channel filter  2012 03 15 fix by an=========================


	if(notif_bss_info->channel>0 && notif_bss_info->channel<15)
		goto filter_ok;
	else if(notif_bss_info->channel >=36 && notif_bss_info->channel <=48)
		goto filter_ok;

	else if(notif_bss_info->channel >=52 && notif_bss_info->channel<=64)
		goto filter_ok;
	else if(notif_bss_info->channel>=100 && notif_bss_info->channel<=140)
		goto filter_ok;
	else if(notif_bss_info->channel>=149 && notif_bss_info->channel<=161)
		goto filter_ok;
	else if(notif_bss_info->channel==165)
		goto filter_ok;
	else
		return -ENOMEM;
	...
	if (notif_bss_info->rssi <=-25 && notif_bss_info->rssi >-30)
		notif_bss_info->rssi+=5;
	if (notif_bss_info->rssi <=-30)
		notif_bss_info->rssi+=13;
