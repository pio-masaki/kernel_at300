...
#define GPS_1V8_REG "vdd_1v8_gps"
#define GPS_3V3_REG "vdd_3v3_gps"
#define GPS_LDO_REG "en_gps_1v8_3v3"
#define GPS_LDO_SRC_REG "vdd_5v0_sys"

struct regulator *gps_1v8_reg = NULL;
struct regulator *gps_3v3_reg = NULL;
struct regulator *gps_ldo_reg = NULL;
struct regulator *gps_ldo_src_reg = NULL;
...
static inline void gps_regulator_enable(int enabled) {
	int ret = 0;

	if (enabled) {
		gps_ldo_src_reg = regulator_get(NULL, GPS_LDO_SRC_REG);
		gps_ldo_reg = regulator_get(NULL, GPS_LDO_REG);
		if (IS_ERR_OR_NULL(gps_ldo_reg) || IS_ERR_OR_NULL(gps_ldo_src_reg)) {
			printk("%s: couldn't get regulator en_gps_1v8_3v3 / vdd_5v0_sys\n", __func__);
			goto regulator_put;
		} else {
			ret = regulator_get_voltage(gps_ldo_reg);
			printk("%s: get volatage %d\n", __func__, ret);
			ret = regulator_set_voltage(gps_ldo_reg, 1800000, 3300000);
			if (ret) {
				printk("%s: couldn't set voltage to 1v8 - 3v3 (%d)\n", __func__, ret);
				goto regulator_put;
			}
			regulator_enable(gps_ldo_src_reg);
			regulator_enable(gps_ldo_reg);
			printk("%s: regulator en_gps_1v8_3v3 enabled\n", __func__);
		}
		
		gps_3v3_reg = regulator_get(NULL, GPS_3V3_REG);
		if (IS_ERR_OR_NULL(gps_3v3_reg)) {
			printk("%s: couldn't get regulator vdd_3v3_gps\n", __func__);
			goto regulator_put;
		}
		regulator_enable(gps_3v3_reg);
		printk("%s: regulator vdd_3v3_gps enabled\n", __func__);
		
		gps_1v8_reg = regulator_get(NULL, GPS_1V8_REG);
		if (IS_ERR_OR_NULL(gps_1v8_reg)) {
			printk("%s: couldn't get regulator vdd_1v8_gps\n", __func__);
			goto regulator_put;
		}
		regulator_enable(gps_1v8_reg);
		printk("%s: regulator vdd_1v8_gps enabled\n", __func__);
	} else {
regulator_put:
		if (gps_1v8_reg != NULL) {
		...
		if (gps_ldo_reg != NULL) {
			regulator_disable(gps_ldo_reg);
			regulator_put(gps_ldo_reg);
		}
	}
}
...
static int __init tegra_uart_probe(struct platform_device *pdev)
{
    ...
    if (u->line == 1) {
        /* Founding uart2, GPS device 
         * Enable gps 1v8 and 3v3 regulator */
        printk("%s: Founding uart2 GPS device\n", __func__);
        gps_regulator_enable(1);
    }
