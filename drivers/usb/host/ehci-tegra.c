...
#define TEGRA_USB_PORTSC1_PTC_MASK				(0xF << 16)
#define TEGRA_USB_PORTSC1_PTC_TEST_NORMAL_OP	(0 << 16)
#define TEGRA_USB_PORTSC1_PTC_TEST_J			(1 << 16)
#define TEGRA_USB_PORTSC1_PTC_TEST_K			(2 << 16)
#define TEGRA_USB_PORTSC1_PTC_TEST_SE0_NAK		(3 << 16)
#define TEGRA_USB_PORTSC1_PTC_TEST_PKT			(4 << 16)
#define TEGRA_USB_PORTSC1_PTC_TEST_FORCE_ENABLE	(5 << 16)
...
static const char test_j[] = "test_j";
static const char test_k[] = "test_k";
static const char test_se0_nak[] = "test_se0";
static const char test_pkt[] = "test_pkt";
static const char test_normal[] = "normal";

static unsigned int test_mode[3] = {0,0,0};
                              ...
                              char *buf)
{
    ...
    if (test_mode[instance] == TEGRA_USB_PORTSC1_PTC_TEST_NORMAL_OP) {
        return sprintf(buf, "%s\n", test_normal);
    } else if (test_mode[instance] == TEGRA_USB_PORTSC1_PTC_TEST_J) {
        return sprintf(buf, "%s\n", test_j);
    } else if (test_mode[instance] == TEGRA_USB_PORTSC1_PTC_TEST_K) {
        return sprintf(buf, "%s\n", test_k);
    } else if (test_mode[instance] == TEGRA_USB_PORTSC1_PTC_TEST_SE0_NAK) {
        return sprintf(buf, "%s\n", test_se0_nak);
    } else if (test_mode[instance] == TEGRA_USB_PORTSC1_PTC_TEST_PKT) {
        return sprintf(buf, "%s\n", test_pkt);
    } else
                               ...
                               const char *buf, size_t count)
{
    ...
    temp = readl(hcd->regs + TEGRA_USB_PORTSC1_OFFSET);
    temp = temp & (~TEGRA_USB_PORTSC1_PTC_MASK);

    if (!strncmp(buf, test_normal, len)) {
        printk("USB normal operation\n");
        temp = temp | TEGRA_USB_PORTSC1_PTC_TEST_NORMAL_OP;
    } else if (!strncmp(buf, test_j, len)) {
        printk("USB Test Mode: test_j\n");
        temp = temp | TEGRA_USB_PORTSC1_PTC_TEST_J;
    } else if (!strncmp(buf, test_k, len)) {
        printk("USB Test Mode: test_k\n");
        temp = temp | TEGRA_USB_PORTSC1_PTC_TEST_K;
    } else if (!strncmp(buf, test_se0_nak, len)) {
        printk("USB Test Mode: test_se0_nak\n");
        temp = temp | TEGRA_USB_PORTSC1_PTC_TEST_SE0_NAK;
    } else if (!strncmp(buf, test_pkt, len)) {
        printk("USB Test Mode: test_pkt\n");
        temp = temp | TEGRA_USB_PORTSC1_PTC_TEST_PKT;
    } else {
        return -EINVAL;
    }

    printk("%s: portsc1=0x%x\n", __FUNCTION__, temp);
    writel(temp, hcd->regs + TEGRA_USB_PORTSC1_OFFSET);
    test_mode[instance] = temp & TEGRA_USB_PORTSC1_PTC_MASK;

    return count;
}

static DEVICE_ATTR(test_mode, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP,
                   test_mode_show, test_mode_store);
...
static int tegra_ehci_probe(struct platform_device *pdev)
{
	...
	#if 0
	/* USB Test Package Mode */
	if(instance == 2) {
		unsigned int temp;
		temp = readl(hcd->regs + TEGRA_USB_PORTSC1_OFFSET);
		writel((temp | TEGRA_USB_PORTSC1_PTC_TEST_PKT),
				(hcd->regs + TEGRA_USB_PORTSC1_OFFSET));
		while(1);
	}
	#endif
    ...
    if (instance != 2) {        /* USB2(dock) should not wakeup system */
    ...
    err = device_create_file(&pdev->dev, &dev_attr_test_mode);
