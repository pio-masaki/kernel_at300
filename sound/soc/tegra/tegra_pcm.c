...
static void tegra_pcm_work(struct work_struct *work)
{
	...
	//printk(KERN_INFO "############### tegra_pcm_work %s state:%s func:%s\n", rtd->codec->name, prtd->running?"True":"False",
	//		prtd->substream->stream?"Capture":"Playback");

	if(rtd->codec && (strncmp(rtd->codec->name, "spdif", 5) != 0))
		tegra_pcm_control(prtd->running, prtd->substream);
}
